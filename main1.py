...
def generate_vastu_matrix(layout, Sq_ft):
    ...
    generate_house_map(layout, Sq_ft)

    return layout


def generate_house_map(layout, Sq_ft):
    # Initialize house dimensions (assuming square shape)
    side_length = Sq_ft ** 0.5  # Assuming square shape for simplicity
    ...
    for room, details in layout.items():
        width = details["size"] ** 0.5
        length = details["size"] ** 0.5
        direction = details["direction"]

        # Draw the room (as a rectangle)
        rect = patches.Rectangle(
            (x_offset, y_offset), width, length, linewidth=1, edgecolor='black', facecolor='lightblue', label=room
        )
        ax.add_patch(rect)
        
        # Update position for next room (you can modify the positioning logic as per your layout)
        y_offset += length
        
    ax.set_aspect('equal', 'box')
    ax.set_title("Generated House Layout (Vastu Compliant)")
    ax.set_xlabel("Feet")
    ax.set_ylabel("Feet")
...
def generate_layout(Sq_ft, requirement):
    ...
    try:
        ...
        # Check if the 'candidates' attribute is a list and contains at least one item
        if hasattr(response, 'candidates') and len(response.candidates) > 0:
            candidate = response.candidates[0]  # Get the first candidate
            content_text = candidate.content.parts[0].text  # Access the text content
            layout = json.loads(content_text.strip())  # Parse it as JSON
            print(layout)
            return layout
        else:
            return {"error": "No candidates found in response"}, 500
