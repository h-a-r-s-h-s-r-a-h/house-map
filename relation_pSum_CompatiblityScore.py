...
from entities import priority_order, entity_scores
    ...
    for i in range(len(room_types)):
        for j in range(i + 1, len(room_types)):
            ...
            if normalized_room1 and normalized_room2:
                ...
                if score > 0: 
                    priority_sum = priority_index.get(normalized_room1, len(priority_order)) + priority_index.get(normalized_room2, len(priority_order))
                    
                    best_pairs.append((room1, room2, score, priority_sum))
    
    best_pairs.sort(key=lambda x: (x[2], -x[3]), reverse=True)
    return best_pairs
